...
let setup_assets hub_path =
  ...
  (* Write lockfile from manifest (resolves name+version to sha256 via
     the package index — no git transport, no rev/subdir). *)
...
(* === Release === *)
(* Removed: run_release. Use scripts/release.sh <version> instead.
   The release workflow (.github/workflows/release.yml) handles GH release
   creation with RELEASE.md notes, binary artifacts, and checksums. *)
