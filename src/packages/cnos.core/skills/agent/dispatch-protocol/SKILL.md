...
**Labels are the sole source of truth for dispatch readiness (cnos#640).** Some design-first issue bodies (filed before cnos#640) carry a legacy hold sentence of the shape *"Not dispatched — status:ready. Design-first; dispatch on explicit operator authorization."* This body prose is **not authoritative and MUST NOT be read as a second gate**. The `status:*` label alone determines dispatch readiness, per Kernel §2.2 "one source of truth per fact" (`cnos.core/doctrine/KERNEL.md`). Duplicating the hold state in body prose let the two drift out of sync: cnos#614 first hit this (label flipped to `status:todo` by hand, body prose never updated, the wake deferred on the apparent contradiction and stranded the cell); cnos#633 recurred in the same session because the cnos#614 learning was captured (an RCA comment) but never mechanized (Kernel §1.2 "MCA over MCI" — *"'Won't repeat' without a mechanism is not a fix"*). This paragraph and the primitive below are that mechanization.
- A wake or δ that encounters a body-hold sentence contradicting the current `status:*` label MUST trust the label and MUST NOT defer, re-litigate, or treat the prose as a competing signal. If the label says `status:todo`, the issue is authorized for dispatch regardless of what stale body prose says.
- New design-first issues MUST NOT be filed with this hold-prose convention at all — `status:ready` alone communicates the hold. Existing issues that still carry the sentence are cleaned opportunistically by the primitive below, not hunted down proactively.
- `cn issues dispatch --issue N` (cnos#640; `src/packages/cnos.issues/commands/issues-dispatch/`) is the operator/human-invoked primitive that performs the `status:ready → status:todo` authorization event and, in the same caller-visible operation, strips a legacy body-hold sentence when one is present — so a human/κ never again has to hand-edit the label and the body as two separately-remembered steps. This command does not change who may authorize dispatch (§1.2 above, unchanged): it is invoked by the same operator/planner-with-explicit-approval actors who could already apply `status:todo` directly. **No dispatch wake or scan/reconciler process invokes this primitive on its own initiative** — it is an operator-facing convenience for performing an already-authorized action correctly, not a new authorization path.

- ❌ Wake or δ defers because the issue body's prose disagrees with the `status:*` label
- ✅ Wake or δ trusts the `status:*` label only; a body-hold sentence, if still present, is informational legacy text, not authoritative
- ❌ A dispatch wake calls `cn issues dispatch` to self-authorize a claim
- ✅ An operator (or a planner quoting explicit human approval, per §1.2 above) runs `cn issues dispatch --issue N`
...
**Protocol-specific FSM override (cnos#575).** The step 4 claim (and, symmetrically, hard-block and release-back-to-queue label transitions elsewhere in a cell's lifecycle) are written above as a direct `gh issue edit` call — this is the generic default that applies absent a protocol-specific issue-state FSM. A concrete protocol package MAY ship its own FSM that owns a subset of lifecycle transitions as data-driven, guard-gated proposals (declarative transition table + evaluator) rather than direct writes; when it has, that protocol's dispatch wake MUST request the transition through its FSM (e.g. `cn issues fsm evaluate --issue {N} --apply`) instead of performing the direct label write shown above, and this skill's generic claim-sequence prose is read as satisfied by that request-then-apply indirection. Today only `cnos.cds` has shipped such an FSM (`cn issues fsm`, cnos#568/#569/#575); its dispatch wake requests all four of its named lifecycle transitions (claim, hard-block, release-back-to-queue, and β-converge) through it — see [`cnos.cds/orchestrators/cds-dispatch/SKILL.md`](../../../../cnos.cds/orchestrators/cds-dispatch/SKILL.md) §"Lifecycle transitions" for the reference shape. Protocols without a package-owned FSM (e.g. `cnos.cdr` today) still perform the direct label write shown in step 4 above — this is the generic default, not a protocol violation. This skill does not mandate that every protocol build an FSM; it only requires that a protocol which HAS one routes its lifecycle transitions through it rather than bypassing it with a direct write.
...
**Detection.** A claim's `run_class` is classified via an ordered decision procedure — `first_pass` / `resumed_from_matter` / `scope_continuation` / `repair_pass`, checked in that fixed priority order, each with its own positive discriminator — canonically defined at [`cnos.cds/orchestrators/cds-dispatch/SKILL.md`](../../../../cnos.cds/orchestrators/cds-dispatch/SKILL.md) §"Repair re-entry preflight" Step A (cnos#639). This framework-level skill does not restate the full procedure (Kernel §2.2 "one source of truth per fact"; §1.2 "derive, do not duplicate") but names `repair_pass`'s outcome: its positive discriminator is direct rejection evidence for issue `#{N}` — prior `status:changes` history, prior β/δ bounce comments naming rejected findings, or an existing repair-contract artifact (`delta-repair.md`) — NOT bare branch/PR/artifact presence, which is `resumed_from_matter` or `scope_continuation` territory instead (see the canonical Step A for the full ordering and all four classes). The wake MUST distinguish first-run implementation from repair-run re-entry, resume-from-matter, and scope-continuation before invoking the cell runtime.
...
**Receipt.** The cycle receipt (and the wake's return token) records `run_class ∈ {first_pass, resumed_from_matter, scope_continuation, repair_pass, manual_delta_repair, blocked}` — the complete, canonical set. The ordered classification procedure and every class's positive discriminator are defined once at [`cnos.cds/orchestrators/cds-dispatch/SKILL.md`](../../../../cnos.cds/orchestrators/cds-dispatch/SKILL.md) §"Repair re-entry preflight" Step A; this skill states the enum, not a competing subset of it (cnos#639).
...
The concrete dispatch wake operationalizes this as a §"Closeout integrity preflight" in the cds-dispatch prompt. A CI guard (`scripts/ci/check-dispatch-closeout-integrity.sh`) asserts the rendered dispatch surface carries this contract, so the failure mode cannot silently return. The empty-review detector itself (`status:review` + no PR + no commits → violation) is proven live against the real FSM transition table by `issuesfsm_test.go`'s `TestAC3_EmptyReviewBlocked` / `TestApply_EmptyReviewStateBlocked` / `TestAC574_ReviewPartialEvidenceBlocked` / `TestAC574_ReviewWithPRStillValid`, not by a bash self-test inside the CI script (cnos#600 consolidation — the script's prior hand-rolled `--self-test` mode duplicated these cases and was folded out in favor of the Go tests, which exercise the same invariant against the production transition table rather than a reimplemented shim). This extends cnos#516 (which fixed repair-context loading): #516 = repair integrity; #524 = deliverable integrity.
...
Confirm §2.9 defines the deliverable-proof requirement before `status:review` and the no-deliverable STOP rule. Run `./scripts/ci/check-dispatch-closeout-integrity.sh` (asserts the dispatch-protocol, the `cds-dispatch` prompt, and the rendered golden + live workflow all carry the `Closeout integrity` / `deliverable_evidence` contract). The check fails if a future edit removes the contract from the prompt/protocol (and thus from the re-rendered golden) — the cnos#524 W4 empty-review failure mode cannot silently return. Confirm the empty-review detector itself (`status:review` + no PR + no commits → violation; PR + commits → ok) is green via `go test ./src/packages/cnos.issues/commands/issues-fsm/... -run 'TestAC3_EmptyReviewBlocked|TestApply_EmptyReviewStateBlocked|TestAC574_ReviewPartialEvidenceBlocked|TestAC574_ReviewWithPRStillValid'` (cnos#600 consolidation — this replaced the CI script's bash `--self-test` mode).
...
- **D12 — Empty review (cell sets `status:review` with no deliverable).** _Symptom:_ A cell claims `status:todo → status:in-progress`, runs to completion, and sets `status:review` with no PR, no commits, no closeout, and no STOP comment (observed cnos#524 W4: a 25-minute dispatch run claimed the issue and marked review while pushing nothing — a false-complete indistinguishable from a finished cell; the operator expects a deliverable that does not exist). _Fix:_ §2.9 — deliverable proof is mandatory before `status:review` (a PR exists + commits beyond base + branch differs from base + `.cdd/unreleased/{N}/` closeout artifacts exist + the receipt names a `deliverable_evidence` block); on missing proof the wake STOPS, posts a `STOP`/`BLOCKED` comment, and does not set `status:review`. The `scripts/ci/check-dispatch-closeout-integrity.sh` guard keeps the contract present in the rendered dispatch surface; the empty-review detector itself is proven live by the Go FSM test suite (see §2.9, cnos#600 consolidation).
- **D13 — Body/label authorization drift.** _Symptom:_ A design-first issue's body carries a legacy hold sentence ("Not dispatched — status:ready ... dispatch on explicit operator authorization.") that contradicts the issue's actual `status:*` label — most commonly the label was flipped to `status:todo` by hand but the body prose was never updated. A wake or δ reading the body sees the apparent contradiction and defers, costing firings (cnos#614 first occurrence; cnos#633 recurred in the same session because the cnos#614 learning was captured in an RCA comment but never mechanized — Kernel §1.2 "MCA over MCI"). _Fix:_ §1.2's "Labels are the sole source of truth for dispatch readiness" paragraph (cnos#640) — the `status:*` label alone gates dispatch readiness; a body-hold sentence is non-authoritative legacy text a wake/δ MUST NOT defer on. `cn issues dispatch --issue N` (`src/packages/cnos.issues/commands/issues-dispatch/`) performs the `status:ready → status:todo` authorization and strips the legacy body sentence as one operator-invoked, caller-visible operation, so the two surfaces cannot drift apart from a hand-edit again.
...
- `cnos#640` — body/label authorization drift (D13 above): cnos#614 first occurrence + RCA, cnos#633 same-session recurrence; landed §1.2's "labels are the sole source of truth for dispatch readiness" paragraph and the `cn issues dispatch --issue N` primitive (`src/packages/cnos.issues/commands/issues-dispatch/`).
