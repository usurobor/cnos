...
> **δ-as-role boundary policy lives in [`delta/SKILL.md`](../delta/SKILL.md).** This section retains the **dispatch-coordinator mechanics** that `operator/SKILL.md` is authoritative for (the `claude -p` invocations, the gate-action confirmation protocol). The role-policy that governs *whether* and *when* these mechanics fire — δ as sole tag-author, gate-fires-on-request-not-observation, the BoundaryDecision enumerants, the override-as-degraded-action doctrine, the verdict-vs-decision distinction — lives in `delta/SKILL.md` §1 "Outward membrane" and §3 "Override." **Release-effector mechanics** (the single-command release script invocation, post-push CI polling, the recovery runbook, merged-branch deletes) **live in [`release-effector/SKILL.md`](../release-effector/SKILL.md)** as of Phase 4c of cnos#366 (cycle/399).
