...
> **δ-as-role boundary policy lives in [`delta/SKILL.md`](../delta/SKILL.md).** This section retains the **dispatch-coordinator + release-effector mechanics** that `operator/SKILL.md` is authoritative for (the `claude -p` and `scripts/release.sh` invocations, the CI-polling commands, the branch-cleanup runbook). The role-policy that governs *whether* and *when* these mechanics fire — δ as sole tag-author, gate-fires-on-request-not-observation, the BoundaryDecision enumerants, the override-as-degraded-action doctrine, the verdict-vs-decision distinction — lives in `delta/SKILL.md` §1 "Outward membrane" and §3 "Override." Phase 4c of cnos#366 will further separate the release-effector mechanics; until then, the mechanics live here.
