...
3. begin polling the issue and `.cdd/unreleased/{N}/` on `origin/cycle/{N}` (see CDD.md §Tracking) — γ may write coordination notes to `.cdd/unreleased/{N}/gamma-closeout.md`, and β will respond in `.cdd/unreleased/{N}/beta-review.md` once you signal review-readiness. **Issue-edit cache-bust:** when polling sees a `gamma-clarification.md` add or update on the cycle branch, re-read the issue body via `gh issue view {N}` (or MCP equivalent) to get the live version, not cached state
