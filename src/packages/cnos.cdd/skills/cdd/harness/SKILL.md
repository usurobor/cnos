...
If δ's recovery (§6.3) carries a SIGTERM-terminated cycle through to a merge and δ (in γ's stead, or on δ's own re-dispatch of γ) is about to declare that recovered cycle closed, δ MUST run the same hard-gate assertion `gamma/SKILL.md §2.10` row 15 requires for a normally-dispatched γ close-out: `gh issue view {N} --json state --jq .state` MUST return `CLOSED` before the closure declaration is written. If the result is not `CLOSED` (e.g. the recovery-authored merge commit's subject lacked a close-keyword per `beta/SKILL.md §"Pre-merge gate"` row 5 — plausible in a recovery path, since the commit subject may have been authored under time pressure by δ rather than β), δ MUST run `gh issue close {N}` and record the discrepancy (pre-assertion state, corrective action, post-correction state) in the same location `gamma-closeout.md` would record it, or in `self-coherence.md §Debt` if `gamma-closeout.md` is not yet being authored at recovery time. A recovered cycle is not "closed" while this row is unresolved — recovery does not exempt a cycle from the closure gate, it only changes who runs it. *Derives from: cnos#368 — both falsification precedents for the close-token gap (#365, #367) occurred on 2026-05-15, the same day as α SIGTERM events; the recovery path had no assertion parity with the normal γ closure gate, so a recovery-adjacent closure could silently reproduce the #367 OPEN-state condition.*
