...
- [`CELL-KINDS.md`](CELL-KINDS.md) — **legacy** cell-kind taxonomy, now the **matter/contract domain vocabulary** (`issue_authoring`, `implementation`, `repair`, `recovery`, `wave`, `cleanup`, `audit`, `release`, `doctrine`, `experiment`, `planning`) — each a matter-domain refinement of the one kernel, **not** a canonical cell type; a cell's **class** (WC/PC/CC) is orthogonal (see `docs/architecture/CELL-RUNTIME.md`; proposed, #627/#628). Filename retained for compatibility ([cnos#570](https://github.com/usurobor/cnos/issues/570)); every terminal cell closeout also carries a mandatory `learning`/`epsilon_observations` section per `CELL-KINDS.md` §"Mandatory terminal learning section" ([cnos#614](https://github.com/usurobor/cnos/issues/614)); `CELL-KINDS.md` §"Process self-improvement loop" names the observe → capture → detect-recurrence → mechanize → verify-non-recurrence loop that section's captured learning is supposed to feed, states today's owner per step, and names detect-recurrence as an open gap ([cnos#640](https://github.com/usurobor/cnos/issues/640))
...
- [`cnos.cds/skills/cds/CDS.md §"Development lifecycle"`](../../../cnos.cds/skills/cds/CDS.md) — 0–13 step table, S0–S13 state machine, branch rule (`cycle/{N}` canonical; γ creates from `origin/main`), branch pre-flight, skill loading tiers
