...
| File | Purpose |
|---|---|
| `cdd-artifact-validate.yml` | Validates the exact `cycle/**` number for merge readiness; main requires a separate exact-cycle post-merge invocation that checks role closeouts plus the nonterminal marker/batch pair, and pre-tag validation uses `--mode release` |
