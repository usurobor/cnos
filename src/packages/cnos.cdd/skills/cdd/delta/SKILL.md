...
| **R[N≥1]** | `gamma-scaffold.md` (unchanged from R0 unless a scaffold-side ambiguity surfaces) + `self-coherence.md` with appended `§R[N]` section (authored by α; AC verification + review-ready signal) + `beta-review.md` with appended `§R[N-1]` verdict (authored by β; verdict + findings if any) |
