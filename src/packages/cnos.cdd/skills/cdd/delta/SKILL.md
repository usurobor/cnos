...
Lands the forward-reference in [`cnos.cds/orchestrators/cds-dispatch/prompt.md`](../../../../cnos.cds/orchestrators/cds-dispatch/prompt.md) (line ~73: *"δ's wake-invoked mode is the contract that lands in cnos#467 Sub 5"*). The reference dispatch wake is `cds-dispatch` (cnos#483; `activation_state: declaration-only` until Sub 5C); δ wake-invoked mode is **protocol-agnostic** at the framework level — any future dispatch wake (`cdr-dispatch`, `cdw-dispatch`) invokes the same δ contract.
...
Seven canonical artifact classes per [`cnos.cds/orchestrators/cds-dispatch/wake-provider.json`](../../../../cnos.cds/orchestrators/cds-dispatch/wake-provider.json) `output_contract.artifact_class_taxonomy`:
...
R[N≥1] iteration is an **internal** cell loop — the cell stays `status:in-progress` while β-α loops continue (per `cds-dispatch/prompt.md` §"Lifecycle transitions" and `dispatch-protocol/SKILL.md` §3.6). External lifecycle states (`status:changes`) are the operator/planner's authority post-`status:review`; β iteration is NOT a `status:changes` transition.

**On converge**, after β writes the final `verdict: converge` and the three closeouts land on the branch, δ writes `.cdd/unreleased/{N}/REVIEW-REQUEST.yml` and runs Guard B (`scripts/ci/check-review-request-preflight.sh --guard-b --live`) against it before writing the `status:review` return token (§9.6) — per cnos#532's review-request proof gate (`dispatch-protocol/SKILL.md` §"Review-request proof gate"). β's converge verdict is necessary but not sufficient: δ MUST NOT write the `status:review` token without a passing Guard B run. On Guard B failure, δ writes `status:blocked` (§9.6) instead, naming the missing proof — converging without provable matter is the cnos#532 / #524 W4 failure class this gate exists to make structurally unable to recur silently.

### 9.6. Return tokens (what δ writes back to the wake)

δ writes back to the wake at four named lifecycle transitions; these map exactly onto `dispatch-protocol/SKILL.md` (cnos#454) §2.4 and [`cnos.cds/orchestrators/cds-dispatch/wake-provider.json`](../../../../cnos.cds/orchestrators/cds-dispatch/wake-provider.json) `responsibilities` #5. δ MUST NOT invent new transitions; δ consumes the dispatch-protocol's transition set.

| Return token | Wake-observable mechanism | When δ writes it |
|---|---|---|
| **status:review** | label transition `status:in-progress → status:review` on the claimed cell + an issue comment naming the cycle-PR URL | After β's `verdict: converge` lands on the branch and all closeouts are present (§9.5 converge boundary) **AND** Guard B passes against `.cdd/unreleased/{N}/REVIEW-REQUEST.yml` (cnos#532). δ opens (or updates) a cycle-PR scoped to the claimed cell, writes `REVIEW-REQUEST.yml`, runs Guard B, then writes the label transition + the PR-URL comment only on a passing Guard B run. On Guard B failure, δ writes `status:blocked` instead (next row) and does NOT transition to `status:review`. |
...
- [`cnos.cds/orchestrators/cds-dispatch/wake-provider.json`](../../../../cnos.cds/orchestrators/cds-dispatch/wake-provider.json) (cnos#483) — the reference dispatch-shape manifest. `output_contract.artifact_class_taxonomy` is the canonical artifact set §9.5 matches; `responsibilities` enumerates the lifecycle transitions §9.6 consumes. `activation_state: declaration-only` until Sub 5C (cnos#487) lands.
- [`cnos.cds/orchestrators/cds-dispatch/prompt.md`](../../../../cnos.cds/orchestrators/cds-dispatch/prompt.md) (cnos#483) — the wake's prompt body; line ~73 forward-references this section; the reference resolves to a landed citation once cnos#486 merges.
