...
| Post-merge closeout validation | δ authorises; γ requests `scripts/validate-release-gate.sh --mode post-merge --cycle N` after role close-outs and the nonterminal post-merge declaration | `operator/SKILL.md` §3.1 |
...
For other gate actions (merge push, branch cleanup), confirm completion to the requesting role per §1.3. But the disconnect tag itself needs no announcement — it speaks for itself.
...
4. route on β's verdict — δ reads β's verdict from the branch state. If `verdict: iterate`, δ re-dispatches α at step 2 with β's findings appended; the cell stays `status:in-progress` (the wake observes via §9.5 token discipline). If `verdict: converge`, δ verifies the three review-time artifacts (`gamma-scaffold.md`, `self-coherence.md`, `beta-review.md`) and writes the `status:review` return token (§9.6). Role closeouts are post-merge and are obtained by the closure flow.
5. after external review acceptance and merge, dispatch the role closeouts — δ re-dispatches α and β for their post-merge closeouts, then γ for post-merge triage and `gamma-closeout.md`. This step is outside the pre-merge converge/`status:review` boundary. **δ does NOT author closeout content**; δ dispatches the role that owns each closeout.
...
| **converge / review-ready** | `gamma-scaffold.md` + `self-coherence.md` (all §R[N] sections; converge-ready signal at the last) + `beta-review.md` (all §R[N] sections; final `verdict: converge`). `REVIEW-REQUEST.yml` separately records δ's transition request and deliverable evidence. No role closeout or PRA is a pre-merge prerequisite. |
...
**On converge**, after β writes the final `verdict: converge` and the three review-time artifacts are present on the branch, δ writes the `status:review` return token (§9.6).
...
### 9.13. scope-continuation shape (cnos#639)

This subsection names a third, structurally distinct resume shape, sibling to §9.10 and §9.11: what δ does when a claimed `status:todo` cell carries an operator (or CAP-authorized) comment authorizing a **new bounded AC scope** on an issue whose prior cycle round(s) already **converged and merged** — not a rejection (§9.10) and not a mechanically-checkpointed dead run (§9.11).

**Trigger.** The operator (or an operator-delegated CAP directive) posts a comment on an already-shipped issue authorizing a new, explicitly bounded scope of work (e.g. "AC3/AC4 continuation dispatched") and re-applies `status:todo`. The prior round(s)' PR(s) already merged into `main` before this comment — the issue's receipt for those ACs is honestly "done." The next wake firing that claims the cell (under `dispatch:cell + protocol:{P} + status:todo`) hands δ a cell whose `cycle/{N}` branch may be freshly reset to `main` (zero commits beyond base — indistinguishable from a brand-new branch by branch-state alone) but whose `.cdd/unreleased/{N}/` history and issue timeline carry the prior converged rounds. This is the `scope_continuation` shape.

**How this differs from §9.10 and §9.11.**

| | §9.10 resumed-from-changes | §9.11 resumed-from-mechanical-reversion | §9.13 scope-continuation (this section) |
|---|---|---|---|
| Cause | Operator read the PR at `status:review` and returned `iterate`/`reject` via `cn cell return` | The prior run died mid-cycle before reaching `status:review`; the recovery scanner's own reconciliation requeued it | Operator/CAP explicitly authorizes a NEW bounded scope on an issue whose prior round(s) already **merged** (not rejected, not stranded) |
| Evidence on the branch | `operator-review.md` present; `self-coherence.md` carries `§R[N]` for N≥1; explicit rejection findings | No `operator-review.md`; branch may carry only a partial R0; scanner's "MECHANICAL reversion" audit-note comment present | No `operator-review.md`, no scanner comment; an operator/CAP continuation-authorization comment naming the new bounded scope; issue-level history of merged PR(s) for prior rounds (branch itself may be reset to a clean `main` base) |
| What "resume" means | Address `operator-review.md`'s findings; append a NEW `§R[N+1]` round | Continue the SAME round from wherever it stopped — do not re-scaffold, do not treat the partial state as a rejection | Do NOT re-scaffold merged matter; treat prior `CDDArtifacts` as valid closed history, not a conflict; append a NEW `§R[N+1]` round bounded to the operator's newly-authorized scope |
| Repair-contract machinery (`cds-dispatch/SKILL.md` §Repair re-entry preflight Steps B–E) | Does not apply (no `status:changes` involved) | Does not apply — Step A excludes this shape before reaching repair-re-entry classification | Does not apply — Step A's check 2 excludes this shape before reaching the repair-re-entry check, precisely so a merged-and-converged prior round is never mistaken for rejected work |

**Detection.** δ detects the scope-continuation shape from the same five-input contract §9.2 already supplies, plus a direct read of the issue's comment history (already part of δ's claimed-issue-number input #1 per §9.2) — no new Go surface is required. Indicators (per `cds-dispatch/SKILL.md` §"Repair re-entry preflight" Step A check 2, ALL required):
- an operator or CAP-authorized comment on the issue explicitly authorizing a new bounded AC scope;
- issue-level evidence that at least one prior round already **merged** — a prior PR referencing `#{N}` that is closed/merged, and/or `CDDArtifacts` non-empty in a fresh `FactSnapshot` even though the `cycle/{N}` branch itself may show zero commits beyond a just-reset `main` base;
- NO operator-rejection evidence (no `operator-review.md`, no prior `status:changes` history) — the negative check that rules out §9.10;
- NO scanner "MECHANICAL reversion" audit-note comment — the negative check that rules out §9.11.

Per `cds-dispatch/SKILL.md` Step A's fixed ordering, this check runs AFTER the §9.11 mechanical-reversion check (whose positive discriminator — the scanner's comment text — is more specific and must be ruled out first) and BEFORE the repair-re-entry check and the `first_pass` terminal fallthrough — critically, BEFORE `first_pass`, because a scope-continuation branch reset to a clean `main` base is otherwise indistinguishable from a brand-new claim by branch state alone (cnos#626 R2's concrete empirical case).

**Routing sequence (scope-continuation).** Follows §9.3 with one structural difference from a first-claim cell, mirroring §9.11 step 1's pattern:

1. **δ reads the existing `.cdd/unreleased/{N}/` and issue history first** — before dispatching any role, δ confirms the prior round(s)' artifacts and merged PR(s) referenced by the operator's continuation comment. δ does NOT dispatch γ for a fresh R0 scaffold: `gamma-scaffold.md` from the original round remains the cell's contract (the new scope is a bounded addendum, not a redefinition), and prior `CDDArtifacts` are read as valid closed history, never treated as a conflict requiring resolution.
2. **δ dispatches α directly for the new round** — α receives: cycle branch HEAD (freshly reset to `main`, if the operator reset it); the original `gamma-scaffold.md`; the operator's continuation-authorization comment naming the new bounded scope; prior `self-coherence.md` §R[0..N] sections (read-only context, not to be re-litigated). α appends `self-coherence.md §R[N+1]` scoped to exactly the operator-authorized AC set, commits, pushes, exits. α does NOT spawn β, and does NOT re-implement or re-verify the already-merged prior rounds' ACs.
3. **δ dispatches β for R[N+1]** — same as §9.3 step 3, scoped to the new round's ACs only.
4. **δ routes on β's R[N+1] verdict** — same as §9.3 step 4.
5. **δ dispatches γ for closeout amendment** — γ appends a `§R[N+1] amendment` section to `gamma-closeout.md` (or, per the β-α-collapse-on-δ protocol commitment observed at cnos#626, γ/α/β authorship may collapse into a single pass for mechanically-verifiable AC oracles); the amendment records the operator's continuation comment as the round's trigger, distinct from §9.10's rejection-repair framing.

**Preserved invariants.**
- `cycle/{N}` branch — the operator MAY reset it to a clean `main` base between rounds (unlike §9.10/§9.11, where δ never rebases); when reset, δ treats this as the expected scope-continuation shape, not as branch loss
- `.cdd/unreleased/{N}/` artifact directory — prior rounds' artifacts are preserved as historical record even across a branch reset (they live in the merged PR history on `main`, not solely on the working branch)
- `gamma-scaffold.md` — not re-authored unless the operator's continuation comment itself requires a scope redefinition (rare; if so, δ dispatches γ explicitly for a scoped amendment, not a full re-scaffold)
- Prior merged rounds' `self-coherence.md`/`beta-review.md` content — not re-litigated; the new round's α/β verify only the newly-authorized ACs

**Empirical anchor.** cnos#626 R1 and R2 are the first empirical witnesses for the scope-continuation shape: an operator/CAP comment authorized AC3 (R1) and then AC4 (R2) as successive bounded scopes on an issue whose AC1/AC2 (R0) had already merged as PR #635 (before R1's dispatch) and whose AC3 (R1) had already merged as PR #637 (before R2's dispatch, branch reset to a clean `main` base for R2). `.cdd/unreleased/626/self-coherence.md` §"run_class note" (R1, and repeated at R2) self-disclosed the taxonomy gap explicitly rather than silently mislabeling — the empirical behavior this section now names formally: α/β at R1/R2 did not re-scaffold the merged R0/R1 matter, did not treat the prior `CDDArtifacts` as a conflict, and bounded their work to exactly the operator-authorized AC set. `.cdd/unreleased/626/gamma-closeout.md` (R1 amendment, R2 amendment) records the `scope_continuation` tentative name twice independently — the direct prior art this section's naming adopts. cnos#639 lands this subsection; future cycles with operator-authorized scope continuations use this named, mechanical routing rather than ad hoc self-disclosure.

**Cross-references.**
- [`cnos.cds/orchestrators/cds-dispatch/SKILL.md`](../../../../cnos.cds/orchestrators/cds-dispatch/SKILL.md) §"Repair re-entry preflight" Step A check 2 — the `run_class: scope_continuation` classification and its fixed-ordering justification relative to the other three claim-time checks.
- §9.10 and §9.11 above — the two sibling resume shapes (operator-rejection-driven and scanner-mechanical-driven, respectively); read together with this section, the three cover the complete "cell shows up at claim time with pre-existing matter" space this framework currently names.
- `.cdd/unreleased/626/self-coherence.md` §"run_class note" (R1, L432–448) and the repeat at R2 (L647–649) — the empirical first-and-second witnesses, and the self-disclosed taxonomy gap this section closes.
- `.cdd/unreleased/626/gamma-closeout.md` L309–315 (R1 amendment) and L382–385 (R2 amendment) — the `scope_continuation` tentative-name prior art this section adopts per cnos#639's own scaffold instruction.
