...
2. Load the sub-skill for the active handoff event:
   ...
   **Subs 2–5 of [cnos#404](https://github.com/usurobor/cnos/issues/404) have landed** (cross-repo via cnos#416; dispatch via cnos#417; mid-flight + artifact-channel via cnos#418; receipt-stream via cnos#419). All five v0.1 sub-skills are canonical at their `cnos.handoff` homes; Sub 6 (cross-reference cleanup + close tracker) remains. See [`docs/extraction-map.md`](../../docs/extraction-map.md) for the per-surface source pin.
