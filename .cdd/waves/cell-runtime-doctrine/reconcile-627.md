...
|---|---|---|---|---|---|
| **S1** | doctrine — ratify `CELL-RUNTIME.md`; four axes + WC/PC/CC deployment classes; demote `CELL-KINDS`; settle CM↔V | **ratified-predecessor** | — | #628 (ratified) | S1 is **RATIFIED** per #628: PRs #629 + #646 merged 2026-07-11, issue #628 closed, `.cdd/unreleased/628/beta-review.md` R1 converged. Fixed external root; consumed by WC-1/WC-2/WC-3b as immutable `repo_artifact` refs; not re-opened. **NOTE:** the "Proposed / Not ratified" banner in `CELL-RUNTIME.md` (and the `CELL-KINDS` / docs-index "proposed" text) at the pinned commit is **STALE-HISTORICAL** doc-debt — superseded by the #628 ratification (bound in `wave.external_roots`), **not** a live source-doctrine conflict. |
