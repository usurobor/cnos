...
1. **AC3/AC4's doctrine additions (`cds-dispatch/SKILL.md` claim-mechanism paragraph, Step A's `resumed_from_matter` classification, `delta/SKILL.md` §9.11) are unverified against a live wake-invoked-δ firing.** This cycle itself runs under bootstrap-δ (a single-session δ-as-γ via Agent tool, per the γ scaffold's own dispatch-mode note), not wake-invoked-δ — per `delta/SKILL.md` §9.1's own table, wake-invoked-δ is "not yet observed in production" as of this cycle. §9.11's routing sequence (δ reads existing `.cdd/unreleased/{N}/` state before re-dispatching γ) is written by direct analogy to §9.10's pattern and Key finding 2's FSM-level confirmation, but — exactly like §9.10 itself, whose only empirical witness (cycle/497) was also executed manually via a bootstrap-exception path — §9.11 has zero empirical witnesses of an actual dispatch-wake firing claiming a cnos#630-resumed cell end-to-end. The doctrine is coherent and internally consistent with the rest of §9, but "coherent prose" and "proven-by-a-live-firing" are different claims; this cycle only proves the former. The next cell that lives through the actual #614-shaped scenario in production (a real dead-run-with-checkpointed-PR cell reconciled by `cn issues fsm scan --apply` and then re-claimed by a real wake firing) is the empirical anchor §9.11 currently lacks, mirroring how §9.10 names cycle/497 as its own first empirical witness.
2. **The audit-note detection mechanism (Step A's "MECHANICAL reversion" phrase check) is comment-text-based, not structured data.** This was a deliberate, scoped choice (Design call 2: doctrine-only, no new Go/JSON surface), consistent with the guardrail against inventing new mechanisms. If prose-based detection proves fragile in practice (e.g. a wake session summarizing rather than quoting the comment, or GitHub comment-formatting drift), a future cycle might need a more structured signal (e.g. a dedicated label or a `.cdd/unreleased/{N}/` marker file mirroring the `CLAIM-REQUEST.yml`/`REVIEW-REQUEST.yml` pattern) — but that would be new mechanism, out of this cycle's scope per the scaffold's binding guardrail ("no new status:* label... reuse existing primitives, no parallel mechanism"), so it is named here as a candidate follow-up rather than built.
3. **A cell that resumes via the cnos#630 path and then dies again mid-cycle is not separately tested end-to-end** (only single-dead-run-event idempotence is directly tested, via `TestScan_Idempotent`'s two-pass shape). Because `Evaluate` is a pure function of `FactSnapshot` with no hidden state, a second dead-run event after a resume is mechanically identical to the first (same rule, same guards, same outcome) — this is a property of the table-driven design, not something this cycle newly proves, but it is also not independently exercised by a "died, resumed, died again" fixture chain. Low risk given the statelessness argument, but named explicitly rather than silently assumed.
4. **This document's "only one pre-existing fixture matches the new rule's guard combination" claim (§ACs, AC5) is a point-in-time audit, not an enforced invariant.** A future fixture added with `pr_exists: true` + dead run + no review/release/block markers will correctly (and intentionally) start matching `propose_status_todo_with_matter` too — that is the design working as intended, not drift — but a reader diffing this document against a much later `testdata/` state should not expect the "22 pre-existing fixtures, only one matches" count to still hold verbatim.
5. **No PR has been opened for `cycle/630` yet.** Per `cds-dispatch/SKILL.md` §"Closeout integrity preflight," a PR referencing `#630` (with commits beyond base) is required before `status:review` is requested — this cycle is still mid-round (α round 1, awaiting β), so PR-opening and the `status:in-progress -> status:review` transition request are correctly out of scope for this document; named here only so the review-readiness signal below is not mistaken for a claim that the full lifecycle (through merge) is complete.

## CDD Trace

1. **Design** — not required, explicit reason: the γ scaffold (`.cdd/unreleased/630/gamma-scaffold.md`) already carries the incoherence statement (the wedge, cited precisely to `scan.go:252-256`), the per-AC oracle list, a fully-specified recommended design (the two-part transitions.json-rule + claim-doctrine mechanism), and both open design calls with the scaffold's own recommendation for each — a separate design artifact would restate this content rather than add to it. This document's §ACs "Open design calls" subsection is the coherence-contract-level record of how each call was resolved.
2. **Coherence contract** — this file's §Gap.
3. **Plan** — not required, explicit reason: the implementation is one new declarative rule in an existing table (`transitions.json`), zero new Go control-flow branches (the new action flows through `scan.go`'s pre-existing generic reconcile case), two doctrine-prose additions following an existing precedent's shape (§9.10 → §9.11), and a bounded set of test additions/redirects fully enumerated by the scaffold's own per-AC oracle list — no multi-step sequencing risk exists that a separate plan artifact would surface beyond what the scaffold's "Recommended design" numbered list (1–3) already sequences.
4. **Tests** — `src/packages/cnos.issues/commands/issues-fsm/issuesfsm_test.go` (4 new `TestAC630_*` functions: `WedgePreFixRuleReproducesStrand`, `WedgeFixResolvesToTodoWithMatterPreserved`, `AuditNoteReasonNamesMechanicalReversion`, `TodoWithExistingBranchAndPRResumesNotDeferred`); `src/packages/cnos.issues/commands/issues-fsm/scan_test.go` (1 redirected test — `TestScan_DeadWithPRNoReviewRequest_FinalizeNoOpNoComment` → `TestScan_DeadWithCheckpointedPR_RequeuesToTodoPreservingMatter`; 1 new coverage-preservation test — `TestScan_DeltaRecoveryObserveApplyRace_FinalizeNoOpNoComment`; `TestScan_Idempotent` updated in place for the `checkpointed` sub-case); `src/packages/cnos.issues/commands/issues-fsm/testdata/todo-resume-existing-matter.json` (new fixture, AC3).
5. **Code** — `src/packages/cnos.cds/skills/cds/fsm/transitions.json` (new `propose_status_todo_with_matter` rule, `in-progress` state); `src/packages/cnos.issues/commands/issues-fsm/table.go` (Action doc-comment update only — no behavioral change); `src/packages/cnos.issues/commands/issues-fsm/scan.go` (comment updates documenting the new rule's flow through the pre-existing generic reconcile branch and the delta-recovery else-branch's changed meaning — no new branch, no new function).
6. **Docs** — `src/packages/cnos.cds/orchestrators/cds-dispatch/SKILL.md` (claim-mechanism paragraph after step 9; Step A `run_class` classification split into 3 cases with `resumed_from_matter` added; `run_class` enum updated) + its rendered artifacts (`.github/workflows/cnos-cds-dispatch.yml`, `src/packages/cnos.cds/orchestrators/cds-dispatch/cnos-cds-dispatch.golden.yml`, both re-rendered via `cn-install-wake`, sha256-verified byte-identical, never hand-edited); `src/packages/cnos.cdd/skills/cdd/delta/SKILL.md` (new §9.11 "resumed-from-mechanical-reversion shape," sibling to §9.10). **Artifact enumeration matches diff** (α SKILL.md §2.6 rule 11): the table in §Self-check above maps every file in `git diff --stat origin/main..HEAD` (10 files, re-verified post-rebase) to the section(s) that name it; no file is unaccounted for.
7. **Self-coherence** — this file, all sections above, written incrementally per `alpha/SKILL.md` §2.5 (one section per commit: §Gap `871a07ef`, §Skills `6fa13080`, implementation `79fc8565`, §ACs `3b45c29a`, §Self-check `625e4feb`, rebase+re-stamp `04922171`, §Debt `efb8dbe0`, this §CDD Trace section next).

**Caller-path trace for new modules (α SKILL.md §2.6 rule 12):** no new Go module/function without a non-test caller — see the identical claim already made in §Self-check above (`propose_status_todo_with_matter` is data consumed by `scan.go`'s pre-existing generic reconcile branch; every new test function's caller is the Go test runner).

**Test assertion count from runner output (α SKILL.md §2.6 rule 13):** `go test ./... -v` from `src/packages/cnos.issues/commands/issues-fsm`, re-run post-rebase: `grep -c '^--- PASS'` = **77**, `grep -c '^--- FAIL'` = **0** (pasted in full in §Self-check above). This is the complete package suite (pre-existing tests + this cycle's 4 new `TestAC630_*` + 1 new `TestScan_DeltaRecoveryObserveApplyRace_*` + 1 redirected + `TestScan_Idempotent` updated in place), not a subset.

**Commit-author identity (α SKILL.md §2.6 rule 14):** `git config user.email` was unset for the α role at this session's start (inherited from the harness). α configured `git config user.name "alpha"` and `git config user.email "alpha@cdd.cnos"` (the canonical elided form per `cnos.cdd/skills/cdd/operator/SKILL.md` §"Git identity for role actors") before the first commit of this cycle's work — `git log --format='%ae' -8` confirms every α commit on `cycle/630` carries `alpha@cdd.cnos` from the start; no retroactive rebase/amend for identity was needed (the `origin/main` rebase performed above was for branch currency, not identity remediation, and did not touch commit authorship).

**γ-artifact presence (α SKILL.md §2.6 rule 15):** `.cdd/unreleased/630/gamma-scaffold.md` is present on `origin/cycle/630` at the literal §5.1 canonical path (`git ls-tree -r HEAD .cdd/unreleased/630/gamma-scaffold.md` confirms; this is the artifact read in full before any implementation work began — see §Gap). Configuration: **γ-artifact at canonical §5.1 path.**

**`cn cdd verify` (I6 gate).** Run from repo root: `cn cdd verify --unreleased --exceptions .cdd/exceptions.yml`. Before this §CDD Trace section was added, the tool correctly hard-FAILed on cycle #630 specifically (`❌ self-coherence.md sections — missing required sections: CDD Trace`) because, per `alpha/SKILL.md` §2.5's own binding-header-form paragraph, a cycle with no `beta-review.md` yet is classified `"small-change"` and validated on the non-lenient path. That FAIL is the expected, correct signal for a self-coherence document mid-write (§CDD Trace not yet committed) — not a defect; re-run after this section lands to confirm cycle #630 clears the check. **Re-run after landing this section:** `## Summary: 214 passed, 0 failed, 137 warnings (351 total)` — cycle #630 now shows `✅ self-coherence.md sections — basic section validation passed`.

## Review-readiness | round 1

- **Base SHA:** `6143b53c9098e415c4e7e83791843bcad2313314` (γ's original pinned scaffold base).
- **Cycle branch rebase (§2.6 rule 1, re-validated immediately before this signal):** `cycle/630` was rebased onto `origin/main` at `e059014804229c43f0595af19916e12a86a3e198` during this round (see §Gap for the commit list) and force-pushed. Re-checked immediately before writing this section: `git fetch origin main && git rev-parse origin/main` = `e059014804229c43f0595af19916e12a86a3e198`, and `git merge-base HEAD origin/main` = the same SHA — `cycle/630` still contains `origin/main`'s current tip; no further drift occurred while writing §Self-check/§Debt/§CDD Trace.
- **Implementation SHA (last code commit before this readiness signal):** `79fc8565c1f52572fd38ca8ffc5be3018140bcf0` ("alpha: cnos#630 -- mechanical exit from the partial-matter in-progress wedge").
- **Local CI equivalents run at/after this SHA, from repo root:**
  - `go build ./...` + `go vet ./...` + `go test ./...` (+ `-race`) from `src/packages/cnos.issues/commands/issues-fsm`: clean; 77 `--- PASS`, 0 `--- FAIL`; race-clean.
  - `go build ./...` + `go vet ./...` + `go test ./...` from `src/go`: clean; all 15 internal packages `ok`.
  - `go build ./...` + `go vet ./...` from the other two `go.work` modules (`cnos.cdd/commands/cdd-verify`, `cnos.issues/commands/issues-map`): clean.
  - `bash scripts/ci/check-dispatch-repair-preflight.sh` and `bash scripts/ci/check-dispatch-closeout-integrity.sh`: both exit 0.
  - `install-wake-golden` gate equivalents (sha256 byte-identity between live workflow and golden; idempotent re-render; YAML-parses; substrate-shape needles): all reproduced locally and pass — see §ACs AC5.
  - `cn cdd verify --unreleased --exceptions .cdd/exceptions.yml` (I6): `214 passed, 0 failed, 137 warnings` — PASSED with warnings (warnings are pre-existing, unrelated to this cycle — stale-artifact warnings on other issues' unreleased cycles).
- **Not independently run (no GitHub Actions runner in this sandbox environment):** the actual `Build`/`Package`/`Binary` GitHub Actions jobs, and the live `install-wake-golden.yml` / `check-dispatch-repair-preflight` / `check-dispatch-closeout-integrity` workflow *runs* (their script/gate logic was run directly and locally, per above, but not via an actual Actions runner). **Branch CI is not independently confirmed green on GitHub Actions for this head commit** — β/δ should confirm the live Actions run before merge, per `alpha/SKILL.md` §2.6 rule 10's explicit-disclosure requirement.
- **γ-artifact:** present at canonical §5.1 path (`.cdd/unreleased/630/gamma-scaffold.md` on `origin/cycle/630`).
- **Open-design-call disclosure:** both resolved with rationale in §ACs above ("Open design calls" subsection) — (1) distinct new action `propose_status_todo_with_matter`, not an in-place edit; (2) doctrine-only for AC3, confirmed (no new Go surface), with the confirming evidence (which `FactSnapshot`/`transitions.json` fields already suffice) stated explicitly.
- **Guardrails verified:** no `cycle/{N}` branch or PR deleted/discarded anywhere in the diff (grep for `branch -D`, `pr close`, or equivalent in the diff: zero hits — the new rule's own `reason` text names branch/PR preservation explicitly, and no code path in the diff calls any git-branch-deletion or PR-close primitive); `applyStatusLabel`/`PostComment`/`cn cell finalize` reused verbatim (zero new label-writer or comment-poster call sites — confirmed by reading every diff hunk in `scan.go`, which adds only comments, no new function); no new `status:*` label (`src/packages/cnos.core/labels.json` untouched, confirmed by `git diff --stat` above showing no such file; the 7 pre-existing statuses are unchanged); `#516`/`#524` guard scripts pass (above); `install-wake-golden` re-render done via the renderer, never hand-edited (§ACs AC5).
- **Known debt:** disclosed explicitly in §Debt above (doctrine additions unverified against a live wake-invoked-δ firing; comment-text-based audit-note detection is a deliberate scope-bounded choice; resumed-then-died-again is argued-safe via `Evaluate`'s purity but not independently fixture-tested; the fixture-count claim is point-in-time; no PR opened yet for this round).

**Ready for β.** All 6 ACs have concrete, re-runnable evidence (test names + exact commands + pasted output); both open design calls are resolved and documented with rationale tied to the actual code, not restated scaffold text; the one class of debt I consider genuinely open for β's judgment (not just disclosure) is whether the doctrine-only resolution of AC3/AC4's "not misread as an orphan" half is *sufficiently* binding prose given it has zero live-firing empirical witnesses yet — flagged explicitly in both §Self-check and §Debt so β does not have to discover it. α will not observe β's verdict directly (bounded dispatch model, `alpha/SKILL.md` §2.8); per the dispatch prompt, α does not dispatch β and stops here.
