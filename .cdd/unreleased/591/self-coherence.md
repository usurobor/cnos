...
The dispatch strand class: "open the PR" and "commit/push matter" were the *final
cognitive-agent steps* of a dispatch run, so a run ending after α (or β) but before those
steps stranded the cell — matter on the branch, no PR, stuck `status:in-progress`
(#368/#504/#568/#571/#574×2). This cycle closes that gap by making checkpoint + PR-open a
mechanical `if: always()` finalizer (`cn cell finalize`) that runs regardless of whether
the cognitive session survives to its last step. (Full detail: §R0 → *Summary of what was
built*.)

## Skills

Loaded per the issue's Tier-3 set: `cdd`, `cdd/issue`, `cdd/issue/proof`, `eng`,
`eng/ship` (TDD — failing fixture first), `eng/evolve` (the mechanism/cognition boundary
move), `eng/process-economics` (mechanical finalizer vs guard-accretion), `eng/test`,
`go`, `write`. Mode: design-and-build, small-change cycle.

## AC Coverage

All ten ACs have oracle-backed evidence — see §R0 → **AC-by-AC verification** below for
the per-AC commands/tests. Summary:

| AC | Verdict | Oracle |
|----|---------|--------|
| AC1 mechanical finalizer exists | ✅ | `CellFinalizeCmd` + `if: always()` workflow step (not prose) |
| AC2 matter detection (4 signals) | ✅ | `TestFinalize_MatterDetection` (6 subtests) |
| AC3 matter → branch+commit+draft PR | ✅ | `TestFinalize_Checkpoint` (`--draft`, `Refs #591`) |
| AC4 idempotent PR create/update | ✅ | `TestFinalize_Idempotent` |
| AC5 no matter → no empty PR | ✅ | `TestFinalize_NoMatterNoOp` + dogfood TEST 1 |
| AC6 PR-open failure → STOP evidence | ✅ | `TestFinalize_PROpenFailure` + `FINALIZE-STOP.md` |
| AC7 draft unless review-ready | ✅ | unconditional `--draft`; `TestFinalize_NeverTouchesLabelsOrFSM` |
| AC8 FSM sole label writer | ✅ | zero label-mutation code in `Finalizer`; grep + test |
| AC9 strand class fixture | ✅ | `TestFinalize_AC9_StrandClassRecovered` |
| AC10 all gates green | ✅ | `go build/test/vet` + repair-preflight + closeout-integrity |

## CDD Trace

- γ scaffold: `.cdd/unreleased/591/gamma-scaffold.md` (commit `a12ed026`).
- α implement + self-coherence: `cn cell finalize` (`cell.go` +505, `cmd_cell.go`,
  `main.go`), tests (`cell_test.go` +397), workflow `if: always()` finalizer step + golden
  regen, `cds-dispatch/SKILL.md` doctrine narrowing (commit `27c9d739`).
- `CLAIM-REQUEST.yml` present (claim routed through the FSM per #575).
- Zero diff in `transitions.json` / FSM Go files; `CellReturnCmd`/`CellResumeCmd`
  unmodified.

## Self-check

Reached REVIEW-READY at R0: all ten ACs oracle-backed; `go build`/`go test`/`go vet`
green; both goldens regenerated byte-identical (sha256 match); AC7/AC8 leak-audit greps
zero against the full renderer; FSM surfaces carry zero diff. The finalizer never mutates
a label and never requests a transition — PR-open (mechanical) and review-request
(FSM-guarded) stay distinct. (Detail: §R0 → *Review-ready signal*.)
