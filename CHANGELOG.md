...
|---------|-----|-------------|--------------|------------------|----------------------------------------|
| v3.15.0 | A   | A           | A            | A                | Version coherence (#22): VERSION file as single source of truth, build-time generated module, first-party packages versioned to match system, lockfile populated from VERSION+cnos_commit, cn update reconciles packages in-hub, runtime contract includes package versions, status/doctor surface drift, tests derive version dynamically, CI gate blocks on mismatch. |
