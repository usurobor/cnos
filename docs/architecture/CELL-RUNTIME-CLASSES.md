...
This distinction frames the κ/α discussion in §8. **κ ≠ α holds unconditionally as role propositions — in State A and State B alike**: κ is the control-plane slot outside the cell and α the producer inside it, always distinct roles, never softened or suspended. The note's target — **State B**, where α is a distinct runtime-executed role and κ sits outside the cell — is the design and holds **unconditionally**. During bootstrap the operator **authorized the Sigma activation-lineage to *execute* the α role** (State A) only because the mechanical Planning-Cell runtime that would execute α does not exist yet; that is one lineage **hosting both the κ and α roles across separate activations (hosting-identity collapse), not role-equality** — an **operator-authorized, explicitly transitional posture retired by the very migration this note specifies**, not a licensed permanent way to run a Planning Cell (§8).
...
| **This note** (#662) | The cell contract envelope; class-specific `V`; CC↔ε; the distinct-`CellClass` FSM extension point; the unconditional κ≠α invariant (as role propositions, State A and State B alike) + the operator-authorized transitional bootstrap posture (hosting-identity collapse, State A, §8); the wave FSM sketch | — |
...
**State A — the operator-authorized transitional bootstrap posture.** The mechanical Planning-Cell runtime that would execute α **does not exist yet**. Because there is no runtime to run α mechanically, the operator **authorized the Sigma activation-lineage to *execute* the α role of the Planning Cell during bootstrap.** This is **not** `κ = α` (role equality): κ and α remain **always distinct roles** — κ the control-plane/herald slot **outside** the cell, α the producer **inside** it — and that relation is **never softened, qualified, or suspended**, in State A or State B. What State A actually carries is **hosting-identity collapse**: one Sigma lineage **hosts both the κ role and the α role across SEPARATE activations** — distinct activations, distinct roles, distinct positions relative to the cell boundary, **only the hosting lineage shared** (tracked by **#664**). The **κ activation remains outside the cell**; a distinct Sigma α activation does the producing inside. This is a legitimate, explicitly transitional posture, a designed stage on the path to State B, **not** a nonconformance, **not** a violation, and **not** a second valid role topology licensed to persist. **What is transitional is the *hosting*, not the role relation:** State A → State B differs in *hosting* (one Sigma lineage hosting both roles vs. the mechanical runtime executing α as a separate hosting identity), **not** in the κ/α relation, which is κ≠α throughout. It is **retired the moment the Planning Cell runs mechanically** — its own α executed by the runtime as a separate hosting identity — at which point the hosting-identity collapse is gone; κ≠α held unconditionally all along. Because State A is transitional **by construction**, it does **not** soften, relativize, or make optional the State-B invariant above: authorized-transitional is a bootstrap stage, not a licensed *permanent* topology. Using the identity model from the Thesis ("Sigma" = activation lineage; κ/α/β/γ/δ + WC/PC/CC = functional roles): the operator directive "Sigma **executing the role of alpha**" is exactly this hosting collapse — one lineage hosting both roles — **not** κ becoming α; the migration from State A to State B **is** the operationalization of the three-cell (WC/PC/CC) agent — once the Planning Cell runs mechanically, its own α is executed by the runtime. Authorization is **not** self-warrant, and it does **not** relax the firebreak: **actor collapse** — one activation performing two cell roles inside one cell boundary (e.g. acting as α and β at once), the case that defeats CCNF's β-independence firebreak — remains **forbidden** in State A exactly as in State B. Hosting-identity collapse is the strictly weaker limitation the bootstrap actually carries: the separate α/β/γ activations run under one shared Sigma lineage (separate activations, one hosting identity), tracked by **#664** — a distinct, weaker limitation than actor collapse, and the reason the State-A *independent* warrant is reduced.

**Provenance / warrant of this bootstrap.** Operator authorization of the State-A posture settles that one Sigma lineage **hosting both the κ and α roles** (hosting-identity collapse) is *permitted* during bootstrap; it does **not** make κ and α the same role, and it does **not** by itself supply the cell's *independent* warrant. Because any *internal* β pass is the same Sigma lineage (the hosting-identity limitation tracked by **#664**), **the internal passes do not by themselves constitute independent-β warrant.** The independent warrant for this matter is the **external β — a review entity outside the Sigma lineage — bound to the exact matter-only review SHA it reviewed (the immutable SHA_M the external β reviewed, §11.6)**, plus the later γ receipt that binds that same SHA and the external review. The reduced *independent* warrant of the authorized State-A posture is discharged by that external review, not by any internal attestation: authorized ≠ self-warranting.

**`CELL-KINDS.md:137` is State-A-accurate as a *hosting* description, scoped, and superseded at State B.** `CELL-KINDS.md:137` ("κ … commonly acts as **α of a planning cell**") is **accurate for State A read as a hosting description**: during bootstrap, **one Sigma lineage hosts both the κ and α roles across separate activations (hosting-identity collapse)**, so the same lineage that carries κ also carries the α activation. Its accuracy is as a **hosting** fact, **not** as role-equality and **not** as "κ acts as / equals α": the roles stay distinct (κ≠α), the κ activation stays outside the cell, and a distinct Sigma α activation produces inside. It is **scoped to State A** — and it is **superseded at State B**, where the Planning Cell runs mechanically, its own α is runtime-executed as a separate hosting identity, and the hosting collapse is gone, so line 137 no longer describes the operating topology. Reconciling that prose — marking line 137 as State-A-bootstrap hosting wording and repointing it to the operational, mechanically-separated α role — is bound to a **bounded downstream doctrine-migration issue** owned by the PC-Wave / later doctrine wave; that issue **repoints and scopes** the line, it does not correct an error. **This PC-D0 does not edit `CELL-KINDS.md`** (§14 records the dependency and owner; §15 non-goal).

**Guard consequence (§12):** "κ editing cell-owned artifacts" is a forbidden action precisely because κ ≠ α/β/γ/δ. κ carries intent into control-plane artifacts (issues, labels, PR-review surfaces); it does not produce, review, or close a cell's matter. In State A, the Sigma lineage hosts both the κ and α roles under explicit operator authorization, but the α *work* is a **distinct Sigma activation**, not the κ activation editing cell matter — **hosting-identity collapse, not actor collapse**. This is an **operator-authorized transitional posture disclosed on the receipts**, not a relaxation of the guard (κ≠α as roles still holds) and not a disclosed nonconformance.
...
**The review artifact itself must be content-bound, not identity-bound (self-consistency with §2/§13).** The evidence chain must be **content-bound at every load-bearing link**, and the β *review artifact* γ binds is a load-bearing link, not an exception. γ binds the β review **by immutable content** — a **content hash + retrievable snapshot** of the review (e.g. the review captured as an immutable receipt artifact and hashed) — **not** by a bare GitHub **comment id / URL**. A GitHub comment (or issue body) is **stable object identity, editable under the same id** (update-comment API; editable/removable history) — exactly the failure §2/§13 reject for a `control_plane` ref whose `revision` is identity-only: it pins the object that carried the review, not the exact bytes reviewed. The full chain is therefore **matter SHA_M (content) → β review (content-hash + snapshot) → γ binds both by content**; a γ receipt that records the β review **only by comment id / URL does not satisfy §11.6**, exactly as an identity-only `control_plane` revision does not satisfy §2. (State-A note: the shipped FSM does not yet enforce either binding mechanically; both are honored by commit discipline and receipt fields — the matter-SHA binding by committing spec-only and naming the SHA, the review-artifact binding by γ capturing + hashing the review rather than pointing at a mutable comment — until a schema/FSM check pins them: a specified, not shipped, guarantee.)
...
- **κ editing cell-owned artifacts** (§8) — forbidden precisely because κ ≠ α/β/γ/δ. This is a **role** guard ("κ must not author α matter"; κ≠α as roles): Sigma executing the α role under State-A authorization does **not** breach it, because the α *work* is a **distinct Sigma activation**, not the κ activation authoring α matter — **hosting-identity collapse, not actor collapse** (§8).
...
| κ boundary (κ ≠ α, κ outside the cell) | Invariant (D2/F3) — **unconditional as role propositions, State A and State B alike** | §8 states κ≠α without qualification, in both states. During bootstrap **one Sigma lineage hosts both the κ and α roles (hosting-identity collapse), roles distinct (κ≠α)** — the operator **authorized the Sigma activation-lineage to *execute* the α role** (State A, no mechanical Planning-Cell runtime yet), which is **not** role-equality and **not** "κ acts as / equals α": the κ activation stays outside the cell, a distinct Sigma α activation produces inside. This is an **operator-authorized, explicitly transitional posture** (transitional in *hosting*, not in the κ/α relation), not a nonconformance and not a licensed permanent topology; it is retired at State B and does not soften the invariant. Authorization ≠ self-warrant: the reduced *independent* warrant is discharged by the **external β bound to the exact matter-only review SHA it reviewed (the immutable SHA_M, §11.6)** + the later γ receipt, not by the internal passes. Actor collapse stays forbidden; the bootstrap carries only the weaker hosting-identity collapse across separate Sigma activations, tracked by #664. |
| `CELL-KINDS.md:137` ("κ commonly acts as α of a planning cell") | **State-A-accurate as a *hosting* description, scoped, superseded at State B** | Accurate for State A read as a **hosting** description: **one Sigma lineage hosts both the κ and α roles (hosting-identity collapse), roles distinct (κ≠α)** — not role-equality, not "κ acts as / equals α." Scoped to State A and superseded at State B, where the PC runs mechanically, its α is a separate hosting identity, and the hosting collapse is gone. The **bounded downstream doctrine-migration issue** (PC-Wave / later doctrine wave) **repoints and scopes** line 137 to State-A-bootstrap hosting wording pointing at the mechanically-separated α role — it does not correct an error. **This PC-D0 does not edit `CELL-KINDS.md`** (§8, §15). |
