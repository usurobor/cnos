...
- [ ] **AC7** Existing `test/cmd/cn_workflow_test.ml` passes unchanged. 21 external references verified by grep (`parse`, `validate`, 6 step variants, 5 issue_kind variants, `issue` record field access) — all resolve through type-equality re-exports.
...
- `test/cmd/cn_workflow_test.ml` — 21 reference sites: `Cn_workflow.parse` × 5, `Cn_workflow.validate` × 4, 6 step variants × pattern match, 5 issue_kind variants × pattern match, `Cn_workflow.issue` × 2 type annotations. All resolve through type-equality re-export.
